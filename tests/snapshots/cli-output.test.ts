...
/**
 * The generated header embeds the CLI version (read from package.json), which
 * changes on every release. Normalize it so these snapshots guard formatting,
 * not the version number — otherwise every version bump breaks them.
 */
function normalizeVersion(content: string): string {
    return content.replace(/(Generated by (?:env2op|op2env) v)[\d.]+/g, "$1X.Y.Z");
}
...
describe("CLI output snapshots", () => {
    ...
    describe("template content", () => {
        test("generated template matches snapshot", () => {
            ...
            expect(normalizeVersion(content)).toMatchSnapshot();
        });
