...
async function fetchSha256(version: string, maxRetries = 60): Promise<string> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        if (response.status === 404 && attempt < maxRetries) {
            // 15s between retries (total ~15 min max wait): npm now "processes" a new version
            // after publish, and its tarball can 404 for several minutes after its metadata is live
